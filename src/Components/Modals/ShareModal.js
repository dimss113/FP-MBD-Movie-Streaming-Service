        ...
        <form className="flex-rows flex-wrap gap-6 mt-6">
          {
            shareData.map((data, index) => (
              <data.shareButton key={index} url={url} quote="Cimiflix">
                <div className="w-12 transitions hover:bg-subMain flex-cols text-lg h-12 bg-white rounded bg-opacity-30">
                  <data.icon className="m-auto" />
                </div>
              </data.shareButton>
            ))
          }
